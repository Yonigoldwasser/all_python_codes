...
parser.add_argument('-dir', '--directory', required=False, help='Specify directory that contains raster files', default=r"C:\Users\User\Documents\CI_small_grid_comparison\20_grid_1_6_15_7_only_grid_reg_model\20_grid_1_6_15_7_only_grid_reg_model_2024-01-16_1705407304679_ci_preds")
...
# parser.add_argument('-f', '--from_values', required=False, nargs='+', type=int, default=[0, 1, 2, 3, 4, 5, 6, 7, 9],
#                     help='set from values to reclassify')
# parser.add_argument('-t', '--to_values', required=False, nargs='+', type=int,
#                     default=[102, 41, 21, 81, 11, 81, 41, 21, 101],
#                     help='set to values to reclassify')
...
output_folder = r"C:\Users\User\Documents\CI_small_grid_comparison\20_grid_1_6_15_7_only_grid_reg_model\20_grid_1_6_15_7_only_grid_reg_model_2024-01-16_1705407304679_ci_preds\Popr"
...
#                 raster = np.where((tile21r == 3) & (tile22r == 3), 7, raster)
#                 raster = np.where((tile21r == 3) & (tile22r == 3) & (tile20r == 3), 7, raster)
...
#                 raster_profile = src1.profile
#                 raster = np.where((tile21r == 2) & (tile22r == 2), 2, raster)
#                 raster = np.where((tile21r == 2) & (tile22r == 1), 2, raster)
#                 raster = np.where((tile21r == 3) & (tile22r == 3), 7, raster)
#                 raster = np.where((tile21r == 0) & (tile22r == 0), 0, raster)
#                 raster = np.where(raster2 == 1, 1, raster)
#
...
#                                 resampling=Resampling.nearest)
#                             ########################################################################Poria######
#                             # Multiply the reprojected input numpy array by the multiplying raster numpy array - apply poria with def custom_multiply_arrays
#                             # reprojected_input_arr = np.where(reprojected_input_arr > 0, 1, 10)
#                             # output_arr = custom_multiply_arrays(src1.read(1).astype(args.dtype), reprojected_input_arr)
#                             # output_arr = np.where(output_arr == 10, 1, output_arr)
#                             # output_arr = np.where(output_arr == 20, 2, output_arr)
#                             # output_arr = np.where(output_arr == 30, 3, output_arr)
#                             # output_arr = np.where(output_arr == 40, 4, output_arr)
#                             # output_arr = np.where(output_arr == 50, 5, output_arr)
#                             # output_arr = np.where(output_arr == 60, 6, output_arr)
#                             # output_arr = np.where(output_arr == 70, 7, output_arr)
...
#                             with rasterio.open(os.path.join(output_folder, 'new_' + save_name + '_PoPr.tif'), 'w',
